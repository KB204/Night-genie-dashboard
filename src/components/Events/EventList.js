...
const EventList = () => {
  ...
  const handleSearch = () => {
    ...
    const filteredCities = cities.filter((city) => {
      return (
        ...
        (typeof city.name === 'string' && city.name.toLowerCase().includes(searchQuery.toLowerCase())) ||
        (typeof city.adress === 'string' && city.adress.toLowerCase().includes(searchQuery.toLowerCase())) ||
        (typeof city.description === 'string' && city.description.toLowerCase().includes(searchQuery.toLowerCase())) ||
        (typeof city.rating === 'string' && city.rating.toLowerCase().includes(searchQuery.toLowerCase())) ||
        (typeof city.date === 'string' && city.date.toLowerCase().includes(searchQuery.toLowerCase())) ||
        (typeof city.link === 'string' && city.link.toLowerCase().includes(searchQuery.toLowerCase())) ||
  ...
  const handleDeleteConfirm = async () => {
    try {
      const response = await fetch(`http://localhost:8000/categorie/api/event/${cityToDelete.id}/`, {
        method: 'DELETE',
        headers: {
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell><strong>Event image</strong></TableCell>
            ...
            <TableBody>
              {(rowsPerPage > 0
                ...
                <TableRow key={city.id}>
                  ...
                  <TableCell>{city.adress}</TableCell>
                  <TableCell>{city.description}</TableCell>
                  <TableCell>{city.rating}★</TableCell>
                  <TableCell>{city.date}</TableCell>
                  <TableCell>
                    {city.photo_events ? (
                      <img src={`http://localhost:8000${city.photo_events}`} alt={city.name} style={{ width: '50px', height: '50px' }} />
                    ) : (
                      'No Image'
                    )}
                  </TableCell>
                  <TableCell>{city.link}</TableCell>
